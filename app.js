(function () {
  ...
  const STORAGE_API_KEY = 'bioGenApiKey';
  const FREE_LIMIT = 3;
  ...
  async function generateWithChatGPT() {
    ...
    const prompt = `Создай креативное био для ${platform}. Профессия: ${prof}. Хобби: ${int}. Тон: ${tone}. Язык: русский.`;
  ...
  function emailBio() {
    alert('Функция отправки на email будет доступна в следующих версиях.');
  }
  ...
  // Обработчик для ссылки в подсказке
  ...
  // ВАЖНО: Кнопка buy-pro теперь работает как обычная ссылка из HTML
  // Мы НЕ добавляем обработчик click, чтобы не перебивать href
