...
(function () {
  ...
  let openModal = (modal) => { modal.style.display = 'flex'; };
...
Язык: русский и английский (два варианта)`;
  ...
  function updateUI() {
    if (isPro()) {
      ...
      upgradeLink.style.color = '#10B981';
      ...
      upgradeLink.style.color = '#4F46E5';
      // Disable pro options
      document.querySelectorAll('.pro-option').forEach(opt => {
        opt.disabled = true;
  ...
  function showResult(bio) {
    ...
    if (!isPro()) {
      ...
      const hint = upgradeLink.parentElement;
      hint.innerHTML = `Бесплатно: ${remaining > 0 ? remaining : 0} генераций сегодня. `;
      hint.appendChild(upgradeLink);
    } else {
      // Reset hint to default
      const hint = upgradeLink.parentElement;
      hint.innerHTML = 'Бесплатно: 3 генерации в день. ';
      hint.appendChild(upgradeLink);
    }
  }
